...
public:
  ...
  static self get_by_name(const std::string &name)
  {
  ...
  static self get_by_nick(const std::string &nick)
  {
  ...
  {
    return detail::make_string(value_->value_name);
  }

  std::string value_nick() const noexcept
  {
    return detail::make_string(value_->value_nick);
  }
};
