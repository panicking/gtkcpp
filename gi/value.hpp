...
template<typename T>
struct gtype<T, typename if_valid_type<decltype(
                    repository::declare_gtype_of<T>::get_type())>::type>
    : public std::true_type
{
...
GI_DECLARE_GTYPE_VALUE(unsigned long, G_TYPE_LONG, ulong)
...
namespace repository
{
// above helper can serve as local var unset cleaner
template<>
struct declare_unset_subclass_of<GValue>
{
  typedef detail::Value type;
};
...
class Value : public gi::detail::GBoxedWrapper<Value, GValue>
{
  typedef gi::detail::GBoxedWrapper<Value, GValue> super_type;
...
public:
  ...
  void allocate()
  {
    if (this->data_)
      return;
    this->data_.reset(g_new0(GValue, 1), _deleter);
  }
  ...
  void copy(Value dest) const { g_value_copy(gobj_(), dest.gobj_()); }

  void reset() { g_value_reset(gobj_()); }

  void unset() { g_value_unset(gobj_()); }

  bool transform(Value dest) const
  {
