...
// void pango_matrix_concat (::PangoMatrix* matrix, const ::PangoMatrix* new_matrix);
void base::MatrixBase::concat (const Pango::Matrix & new_matrix) noexcept
{
  ...
  auto new_matrix_to_c = gi::unwrap (new_matrix, gi::transfer_none, gi::direction_in);
...
std::tuple<gdouble, gdouble> base::MatrixBase::get_font_scale_factors () const noexcept
{
  ...
  return std::make_tuple (xscale_o, yscale_o);
}
